...
{
    class LaserSwitchManager
    {
        // アプリケーションログ
        private ApplicationLog appLog = null;
        // レーザースイッチ押下イベント
        public event EventHandler<EventArgs> laserSwitchPushed;
        // レーザースイッチ復帰イベント
        public event EventHandler<EventArgs> laserSwitchPulled;
        // カウンタボード
        ...
        public Boolean initialize()
        {
            ...
            this.appLog.Info("初期化完了 ---レーザースイッチ管理---");
        ...
        private void OnDioInterrupted(object sender, CounterBoardEventArgs e)
        {
            ...
            {
                // 立ち上がりで レーザースイッチプル
                if (laserSwitchPulled != null)
                {
                    Task tsk = Task.Factory.StartNew(() =>
                    {
                        // レーザースイッチプルイベントをコール
                        laserSwitchPulled(this, new EventArgs());
                    });
                }
            }
            ...
            {
                // 立ち下がりで レーザースイッチプッシュ
                if (laserSwitchPushed != null)
                {
                    Task tsk = Task.Factory.StartNew(() =>
                    {
                        // レーザースイッチプッシュイベントをコール
                        laserSwitchPushed(this, new EventArgs());
                    });
                }
            }
        }
